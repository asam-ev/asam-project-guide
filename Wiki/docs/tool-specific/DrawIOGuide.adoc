...
:library-repo: https://code.asam.net/common/asam-graphic-library/-/raw/master/ASAM_graphics_library.xml
...
== Overview
Draw.io is a free tool for creating diagrams. The tool can be downloaded and run locally (as link:{download-link-drawio}[Draw.io Desktop]), accessed through a web browser (through link:{web-link-drawio}[Draw.io web link with ASAM library]), or be used through the Visual Studio Code extension (id: hediet.vscode-drawio). 
...
Draw.io can be used either locally (through the dedicated app <<Local Setup (Desktop),Draw.io Deskop>> or through the <<Local Setup (VSCode Extension),VS Code Extension>>) or with the <<Remote Setup, web app>>. 

=== Local Setup (Desktop)

image::https://dashboard.snapcraft.io/site_media/appmedia/2019/08/android-chrome-512x512.png[width=80,link="{download-link-drawio}",role="right"]

The desktop version can be downloaded as an installation package (requires installation) or a portable version (requires no installation). Both can be downloaded from link:{download-link-drawio}[here].

=== Local Setup (VSCode Extension)

include::Recommended-VSCode-Extensions.adoc[tag=drawio-extension] 

=== Remote Setup

The use of Draw.io through the web browser does not require any setup. Just use the provided link:{web-link-drawio}[link].

== Adding The Custom ASAM Draw.io Library

ASAM provides a custom Draw.io library which contains custom standard image elements and visualization structures that can be reused in new diagrams. The repository of this library is {library-repo}.

=== Add Library To Draw.io Desktop

image::DrawioImportLibrary.gif[title=Import ASAM Draw.io library into Desktop.]

. Download the library from the link:{library-repo}[repository] on GitLab.
...
* link:Recommended-VSCode-Extensions.adoc[Recommended Visual Studio Code Extensions]
* link:https://drawio-app.com/tutorials/[Online tutorials on Draw.io]
* link:https://code.asam.net/common/asam-graphic-library[Repository for custom ASAM Draw.io library]
