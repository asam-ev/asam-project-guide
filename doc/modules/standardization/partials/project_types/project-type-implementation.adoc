...
Examples for tools are checkers for file formats and __{api}__s, add-ons or plug-ins for de-facto standard IDEs (e.g. MATLAB, Simulink, Eclipse, etc.), importers, exporters or reference implementations.
...
Upon request, __{asam}__ provides funding up to 100% of the project's required budget.
...
If the ASAM product shall not be further developed, then __{asam}__ shall publicly announce that the development of the product is discontinued.
