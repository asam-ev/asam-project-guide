// tag::mandatory[]
include::tooling:partial$_attributes.adoc[]
.Mandatory Tools
[cols="^3s,8,^1,3"]
[frame=none,grid=rows]

|===
h|Name|Description|Location|Link

|{gitlab-icon-norole} +
{gitlab-url}[GitLab^]
|All source code and standard relevant documentation. Issues relevant to deliverables should be created here.
|ASAM
|{gitlab-url}[Acess through Webbrowser^] +
xref:tooling:gitlab.adoc[GitLab Tooling]

|{calendar-icon-norole} +
{calendar-url}[OpenX Calendar^]
|Shared calendar for all OpenX projects.
|ASAM
|xref:tooling:calendar.adoc[OpenX Calendar]

|{teams-icon-norole} +
{teams-url}[Microsoft Teams^]
|Chat, conferencing, document folder for sharing documents (e.g. research papers, notes, videos, anything that does not need to be versioned).
|ASAM
|xref:tooling:teams-and-sharepoint.adoc[Teams and Sharepoint Tooling]

|{sharepoint-icon-norole} +
{sharepoint-url}[Microsoft Sharepoint^]
|Sharepoint is a Microsoft service that is used in the background by Teams to manage files. It can be accessed directly or through Teams, which is why all files that have been added either way can be retrieved through both tools.
|ASAM
|xref:tooling:teams-and-sharepoint.adoc[Teams and Sharepoint Tooling]

|===

// end::mandatory[]

// tag::recommended[]
include::tooling:partial$_attributes.adoc[]
.Recommended Tools
[cols="^3s,8,^1,3"]
[frame=none,grid=rows]
|===
h|Name|Description|Location|Link

|{git-icon-norole} +
{git-url}[Git^]
|The version control system ASAM uses. +
|Local / {gitlab-url}[GitLab^]
|xref:tooling:git.adoc[Git Tooling]


|{vscode-icon-norole} +
{vscode-url}[VSCode^]
|An all-in-one open-source text editor that makes life a lot easier.
|Local
|xref:tooling:vscode.adoc[Visual Studio Code Tooling]


|{drawio-icon-norole} +
{drawio-url}[Draw.io^]
|An editor for diagrams. Can be downloaded or access through the browser. Also has a (limited) VS Code extension.
|Local / https://app.diagrams.net/?splash=0&clibs=Uhttps%3A%2F%2Fcode.asam.net%2Fcommon%2Fasam-graphic-library%2F-%2Fraw%2Fmaster%2FASAM_graphics_library.xml[Web^]
|xref:tooling:drawio.adoc[Draw.io Tooling]

|{docker-icon-norole} +
{docker-url}[Docker Desktop^]
|(For local rendering of AsciiDoc) +
A tool for hosting containers.
|Local
|xref:tooling:docker.adoc[Docker Tooling]
|===

// end::recommended[]

