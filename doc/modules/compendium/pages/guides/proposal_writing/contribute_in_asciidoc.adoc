...
However, in order for a consistent workflow and optimal use of the automation steps set up by {asam}, users must first familiarize themselves with the instructions on <<The GitLab workflow,GitLab>> and <<Writing content in AsciiDoc,AsciiDoc>> and follow them.
...
Users **must** follow the xref:compendium:writing_guidelines/asciidoc_guide.adoc[].