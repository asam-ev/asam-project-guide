= ASAM developer certification of origin for open source projects
:description: Contains the ASAM DCO and describes its use.
:keywords: TODO
...
The __{dco}__ is an attestation attached to every contribution made by every developer.
In the commit message of the contribution, (described more fully later in this document), the developer simply adds a Signed-off-by statement and thereby agrees to the __{dco}__.
...
The __{dco}__ requires a sign-off message in the following format appear on each commit in the pull request:
[source]
----
Signed-off-by: Firstname Lastname <email@address.com>
----

The __{dco}__ text can either be manually added to your commit body, or you can add either `-s` or `--signoff` to your usual Git commit commands.
