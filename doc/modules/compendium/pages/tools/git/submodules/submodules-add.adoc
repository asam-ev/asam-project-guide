...
+
. Edit .gitmodule with any text editor (e.g. {vscode}) to change the remote url to the local path. This is needed for the GitLab pipeline to checkout the submodule without access token:
+
[source,git]
----
// Before editing
