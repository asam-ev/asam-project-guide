...
This guide assumes you already have Docker installed and set up. If not, see xref:general_guidelines/Getting-Docker.adoc[Setting Up Docker For Asciidoc] first.

For the default docker-compose.yml needed to easily create the html result of a project locally, see the xref:{mydir}StandardDockerConfig.adoc[default docker-compose.yml for creating local htmls].
...
== See Also
* https://docs.docker.com/get-started/[Official guide for Docker^]
* xref:general_guidelines/Getting-Docker.adoc[Setting Up Docker For Asciidoc]
* xref:{mydir}StandardDockerConfig.adoc[Default docker-compose.yml for creating local htmls]
